...
# CoT variant: enable 768 thinking tokens for vanilla answer generation.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run vanilla AVUT baseline with Chain-of-Thought (768 thinking tokens)."
    )
    parser.add_argument(
        "--input",
        required=False,
    ...
    parser.add_argument(
        "--no-prefetch-videos",
        action="store_true",
        help="Skip Hugging Face video prefetch (baseline video QA will fail without video_input).",
    )
...
def main() -> None:
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join("outputs", "vanilla_cot")
